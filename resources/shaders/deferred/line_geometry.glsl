...
layout(location = 2) out vec4 g_currClip;
layout(location = 3) out vec4 g_prevClip;
...
void main() {
    ...
    g_currClip = currClip;
    g_prevClip = camera.prevViewProj * worldPos; 
    ...
    g_tangent = normalize(mat3(transpose(inverse(mv))) * tangent);
...
layout(location = 2) in vec4 g_currClip;
layout(location = 3) in vec4 g_prevClip;
