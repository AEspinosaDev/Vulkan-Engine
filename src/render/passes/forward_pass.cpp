...
void ForwardPass::setup_out_attachments(std::vector<Graphics::AttachmentConfig>& attachments, std::vector<Graphics::SubPassDependency>& dependencies) {
...
void ForwardPass::create_framebuffer() {
    ...
    if (m_aa == MSAASamples::x1) // IF NOT MULTISAMPLED
        for (size_t fb = 0; fb < m_framebuffers.size(); fb++)
            m_framebuffers[fb] = m_device->create_framebuffer(m_renderpass, m_outAttachments, m_imageExtent, m_framebufferImageDepth, fb);

    else
    {
        std::vector<VKFW::Graphics::Image*> allAttachments{
            &m_interAttachments[0], &m_interAttachments[1], m_outAttachments[0], m_outAttachments[1], m_outAttachments[2]};
        for (size_t fb = 0; fb < m_framebuffers.size(); fb++)
            m_framebuffers[fb] = m_device->create_framebuffer(m_renderpass, allAttachments, m_imageExtent, m_framebufferImageDepth, fb);
    }
}
void ForwardPass::setup_uniforms(std::vector<Graphics::Frame>& frames,const Render::Resources& shared ) {
    ...
    m_descriptorPool.set_layout(GLOBAL_LAYOUT, {camBufferBinding, sceneBufferBinding, shadowBinding, envBinding, iblBinding, accelBinding, noiseBinding});
    ...
    for (size_t i = 0; i < frames.size(); i++)
    {
        ...
        m_descriptors[i].objectDescritor.update(&frames[i].objectBuffer,
...
void ForwardPass::execute(const Render::RenderView& view, const Render::Resources& shared) {
    PROFILING_EVENT()

    if (m_aa > MSAASamples::x1)
        for (size_t i = 0; i < 2; i++)
            m_interAttachments[i].config.clearValue = m_outAttachments[i]->config.clearValue;
    ...
    if (!view.drawCalls.empty())
    {
        for (const auto& drawCallIdx : view.opaqueDrawCalls)
        {
            ...
            ShaderPass* shaderPass = m_shaderPasses[drawCall.shaderID];
        ...
        for (const auto& drawCallIdx : view.transparentDrawCalls)
        {
            auto drawCall = view.drawCalls[drawCallIdx];
...
void ForwardPass::update_uniforms(const Render::RenderView& view, const Render::Resources& shared) {
...
void ForwardPass::link_input_attachments() {
    for (size_t i = 0; i < m_descriptors.size(); i++)
    {
        m_descriptors[i].globalDescritor.update(m_inAttachments[0],

                                                //   VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
                                                LAYOUT_SHADER_READ_ONLY_OPTIMAL,

                                                2);
        m_descriptors[i].globalDescritor.update(m_inAttachments[1], LAYOUT_SHADER_READ_ONLY_OPTIMAL, 3);
        m_descriptors[i].globalDescritor.update(m_inAttachments[2], LAYOUT_SHADER_READ_ONLY_OPTIMAL, 4);
    }
}
void ForwardPass::setup_material_descriptor(Core::IMaterial* mat) {
    // if (!mat->get_texture_descriptor().handle)
    ...
    //         {
    //             mat->get_texture_descriptor().update(get_image(texture), LAYOUT_SHADER_READ_ONLY_OPTIMAL, pair.first);
    ...
    //             mat->get_texture_descriptor().update(get_image(ResourceManager::FALLBACK_TEXTURE), LAYOUT_SHADER_READ_ONLY_OPTIMAL, pair.first);
