...
void TAAPass::setup_uniforms(std::vector<Graphics::Frame>& frames, const Render::Resources& shared) {
...
void TAAPass::execute(const Render::RenderView& view, const Render::Resources& shared) {

    auto cmd = view.commandBuffer;
    ...
    /*RESOLVE FINAL IMAGE*/
    PostProcessPass::execute(view, shared);
    ...
    cmd.blit_image(*m_framebuffers[m_isDefault ? view.presentImageIndex : 0].attachmentImagesPtrs[0], m_interAttachments[0], FILTER_NEAREST);

    cmd.pipeline_barrier(*m_framebuffers[m_isDefault ? view.presentImageIndex : 0].attachmentImagesPtrs[0],
                         LAYOUT_TRANSFER_SRC_OPTIMAL,
