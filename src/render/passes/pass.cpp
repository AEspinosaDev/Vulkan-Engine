...
BasePass::BasePass( const ptr<Graphics::Device>& device, const ptr<Render::GPUResourcePool>& shared, Extent2D extent, bool isGraphical, bool isResizeable, bool isDefault, std::string name )
    : m_device( device )
    , m_shared( shared )
    , m_name( name )
