...
void GeometryPass::setup_out_attachments(std::vector<Graphics::AttachmentConfig>& attachments, std::vector<Graphics::SubPassDependency>& dependencies) {
    ...
    // Depth
    attachments[4] =
        Graphics::AttachmentConfig(m_depthFormat,
                                   1,
...
void GeometryPass::setup_uniforms(std::vector<Graphics::Frame>& frames, const Render::Resources& shared) {
    ...
    for (size_t i = 0; i < frames.size(); i++)
    {
        ...
        m_descriptors[i].globalDescritor.update(get_image(shared.fallbackTexture2D), LAYOUT_SHADER_READ_ONLY_OPTIMAL, 7);
...
void GeometryPass::execute(const Render::RenderView& view, const Render::Resources& shared) {
    PROFILING_EVENT()
    ...
    for (const auto& drawCallIdx : view.opaqueDrawCalls)
    {
        ...
        if (prevTopology != drawCall.params.topology)
        {
            // Choose shader pass based on topology
            switch (drawCall.params.topology)
            {
            ...
            prevTopology = drawCall.params.topology;
        }
...
void GeometryPass::update_uniforms(const Render::RenderView& view, const Render::Resources& shared) {
