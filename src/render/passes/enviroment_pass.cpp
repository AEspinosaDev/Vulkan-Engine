...
void EnviromentPass::setup_uniforms(std::vector<Graphics::Frame>& frames, const Render::Resources& shared) {
...
void EnviromentPass::execute(const Render::RenderView& view, const Render::Resources& shared) {
    if (!view.updateEnviroment)
    {
        if (m_framebuffers[1].attachmentImagesPtrs[0]->currentLayout == LAYOUT_UNDEFINED)
        {
            CommandBuffer cmd = view.commandBuffer;
            cmd.pipeline_barrier(*m_framebuffers[1].attachmentImagesPtrs[0], LAYOUT_UNDEFINED, LAYOUT_SHADER_READ_ONLY_OPTIMAL, ACCESS_NONE);
        }
        return;
    }
    ...
    // Core::SkySettings skySettings = scene->get_skybox()->get_sky_settings();
    // Core::SkySettings skySettings{};
    ...
    int panoramaType = static_cast<int>(0);
...
void EnviromentPass::update_uniforms(const Render::RenderView& view, const Render::Resources& shared) {

    auto drawCall = view.drawCalls[view.enviromentDrawCall];
    m_envDescriptorSet.update(drawCall.textureBatch.back().img, LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0);
