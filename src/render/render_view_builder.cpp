...
void RenderViewBuilder::update_global_data( const ptr<Graphics::Device>& device,
    ...
    */
    ...
    currentFrame->get_ubo( m_global_UBO_Key ).upload_data( &camData, sizeof( CameraUniforms ), 0 );
    ...
    */
    ...
    currentFrame->get_ubo( m_global_UBO_Key ).upload_data( &sceneParams, sizeof( SceneUniforms ), device->pad_uniform_buffer_size( sizeof( CameraUniforms ) ) );
}
        ...
        for ( Core::Mesh* m : scene->get_meshes() )
        {
            if ( m ) // If mesh exists
            {
                if ( m->is_active() && m->get_geometry() )
                {
                    ...
                    // Offset calculation
                    uint32_t objectOffset = currentFrame->get_ubo( m_object_UBO_Key ).strideSize * mesh_idx;
                    ...
                    currentFrame->get_ubo( m_object_UBO_Key ).upload_data( &objectData, sizeof( ObjectUniforms ), objectOffset );
                    ...
                    drawCall.bufferOffset = currentFrame->get_ubo( m_object_UBO_Key ).strideSize * mesh_idx;

                    // ObjectUniforms materialData;
                    MaterialUniforms materialData = mat->get_uniforms();
                    currentFrame->get_ubo( m_object_UBO_Key ).upload_data( &materialData, sizeof( MaterialUniforms ), objectOffset + device->pad_uniform_buffer_size( sizeof( MaterialUniforms ) ) );
