...
void VKFW::Tools::Loaders::load_PLY( Core::Mesh* const mesh, const std::string fileName, bool preload, bool verbose, bool calculateTangents ) {
...
void VKFW::Tools::Loaders::load_3D_file( Core::Mesh* const mesh, const std::string fileName, bool asynCall ) {
    ...
    if ( dotPosition != std::string::npos )
    {
        ...
        if ( fileExtension == OBJ )
        {
            if ( asynCall )
            {
                std::thread loadThread( Loaders::load_OBJ, mesh, fileName, false, true );
                loadThread.detach();
            } else
                Loaders::load_OBJ( mesh, fileName, false, true );
        ...
        if ( fileExtension == PLY )
        {
            if ( asynCall )
            {
                std::thread loadThread( Loaders::load_PLY, mesh, fileName, true, false, true );
                loadThread.detach();
            } else
                Loaders::load_PLY( mesh, fileName, true, false, true );
...
void VKFW::Tools::Loaders::load_texture_2D( Core::ITexture* const texture, const std::string fileName, bool asyncCall ) {
    size_t dotPosition = fileName.find_last_of( "." );
    ...
    if ( dotPosition != std::string::npos )
    {
        ...
        if ( fileExtension == PNG || fileExtension == JPG || fileExtension == "jpeg" )
        {
            if ( asyncCall )
            {
                ...
                Loaders::load_PNG( texture, fileName );
        ...
        if ( fileExtension == HDR )
        {
            if ( asyncCall )
            {
                std::thread loadThread( Loaders::load_HDR, texture, fileName );
                loadThread.detach();
            } else
                Loaders::load_HDR( texture, fileName );
...
void VKFW::Tools::Loaders::load_PNG( Core::ITexture* const texture, const std::string fileName ) {
    int            w, h, ch;
