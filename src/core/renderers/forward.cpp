...
void ForwardRenderer::setup_renderpasses()
{
    ...
    // Shadow Pass
    graphics::ShadowPass *shadowPass = new graphics::ShadowPass(&m_context, {SHADOW_RES, SHADOW_RES}, totalImagesInFlight,
                                                            VK_MAX_LIGHTS, m_settings.depthFormat);
    m_renderPipeline.push_renderpass(shadowPass);

    // Forward Pass
    graphics::ForwardPass *forwardPass =
        new graphics::ForwardPass(&m_context, m_window->get_extent(), totalImagesInFlight, m_settings.colorFormat,
    ...
    // FXAA Pass
    graphics::FXAAPass *fxaaPass = new graphics::FXAAPass(&m_context, m_window->get_extent(), totalImagesInFlight,
