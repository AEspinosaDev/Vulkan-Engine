...
void ForwardRenderer::setup_renderpasses()
{
    ...
    // Shadow Pass
    ShadowPass *shadowPass = new ShadowPass(&m_context, {SHADOW_RES, SHADOW_RES}, totalImagesInFlight, VK_MAX_LIGHTS,
                                            m_settings.depthFormat);
    m_renderPipeline.push_renderpass(shadowPass);

    // Forward Pass
    ForwardPass *forwardPass =
        new ForwardPass(&m_context, m_window->get_extent(), totalImagesInFlight, m_settings.colorFormat,
    ...
    // FXAA Pass
    FXAAPass *fxaaPass = new FXAAPass(&m_context, m_window->get_extent(), totalImagesInFlight, m_settings.colorFormat,
                                      m_vignette, m_settings2.fxaa);
