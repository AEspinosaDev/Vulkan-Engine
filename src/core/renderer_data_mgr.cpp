		...
		for (Mesh *m : scene->get_meshes())
		{
			if (m) // If mesh exists
			{
					...
					m->get_bounding_volume()->is_on_frustrum(scene->get_active_camera()->get_frustrum())) // Check if is inside frustrum
				{
					...
					m_context.frames[m_currentFrame].uniformBuffers[1].upload_data(m_context.memory, &objectData, sizeof(ObjectUniforms), objectOffset);
...
void Renderer::init_resources()
{
	for (size_t i = 0; i < m_context.frames.size(); i++)
	{
		...
		globalBuffer.init(m_context.memory, globalStrideSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU, (uint32_t)globalStrideSize);
		...
		objectBuffer.init(m_context.memory, VK_MAX_OBJECTS * objectStrideSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU, (uint32_t)objectStrideSize);
