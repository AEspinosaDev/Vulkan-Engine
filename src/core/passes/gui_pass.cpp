...
namespace Core {}
void Core::GUIPass::setup_out_attachments(std::vector<Graphics::AttachmentConfig>&  attachments,
...
void Core::GUIPass::clean_framebuffer() {
   CHECK_INITIALIZATION()
    for (Graphics::Framebuffer& fb : m_framebuffers)
        fb.cleanup();
}

void Core::GUIPass::execute(Graphics::Frame& currentFrame, Scene* const scene, uint32_t presentImageIndex) {
    PROFILING_EVENT()
