...
void ForwardRenderer::create_passes() {
    ...
    m_passes[SKY_PASS]        = std::make_shared<Render::SkyPass>(m_device, skyPassConfig, Extent2D{1024, 512});
    ...
    m_passes[FORWARD_PASS] = std::make_shared<Render::ForwardPass>(m_device, forwardPassConfig, DISPLAY_EXTENT, HDR_FORMAT, DEPTH_FORMAT, m_settings.samplesMSAA);
    m_passes[BLOOM_PASS]   = std::make_shared<Render::BloomPass>(m_device, bloomPassConfig, DISPLAY_EXTENT);
    ...
    m_passes[TONEMAPPIN_PASS] =
        std::make_shared<Render::TonemappingPass>(m_device, toneMappingPassConfig, DISPLAY_EXTENT, m_settings.displayColorFormat, !m_headless);
