...
void ForwardRenderer::on_before_render(Core::Scene* const scene) {
    ...
    if (scene->get_skybox())
    {
        if (scene->get_skybox()->update_enviroment())
            static_cast<Core::ForwardPass*>(m_renderpasses[FORWARD_PASS])
    ...
    m_renderpasses[FORWARD_PASS]->set_attachment_clear_value(
...
void ForwardRenderer::on_after_render(RenderResult& renderResult, Core::Scene* const scene) {
    ...
    if (m_updateShadows)
    {
        ...
        connect_renderpass(m_renderpasses[FORWARD_PASS]);
    }
}
void ForwardRenderer::create_renderpasses() {
    ...
    m_renderpasses.resize(3, nullptr);
    // Shadow Pass
    ...
    // Forward Pass
    m_renderpasses[FORWARD_PASS] = new Core::ForwardPass(&m_device,
                                                         m_window->get_extent(),
                                                         totalImagesInFlight,
    ...
    m_renderpasses[FORWARD_PASS]->set_image_dependace_table({{SHADOW_PASS, {0}}});
    ...
    m_renderpasses[FXAA_PASS] = new Core::FXAAPass(&m_device,
                                                   ...
                                                   m_softwareAA);
    m_renderpasses[FXAA_PASS]->set_image_dependace_table({{FORWARD_PASS, {0}}});
    if (!m_softwareAA)
