...
void ForwardRenderer::create_passes() {
    ...
    Render::PassLinkage<3, 3> forwardPassConfig     = { m_attachments, { 3, 1, 2 }, { 5, 6, 7 } };
    Render::PassLinkage<2, 1> bloomPassConfig       = { m_attachments, { 5, 6 }, { 8 } };
    Render::PassLinkage<1, 1> FXAAPassConfig        = { m_attachments, { 8 }, { 9 } };
    Render::PassLinkage<1, 1> toneMappingPassConfig = { m_attachments, { 9 }, { 10 } };

    // Create passes
    //--------------------------------

    m_passes[SKY_PASS]        = std::make_shared<Render::SkyPass>( m_device, m_shared, skyPassConfig, Extent2D { 1024, 512 } );
    ...
    m_passes[FORWARD_PASS]    = std::make_shared<Render::ForwardPass>( m_device, m_shared, forwardPassConfig, DISPLAY_EXTENT, HDR_FORMAT, DEPTH_FORMAT, m_settings.samplesMSAA );
    m_passes[BLOOM_PASS]      = std::make_shared<Render::BloomPass>( m_device, m_shared, bloomPassConfig, DISPLAY_EXTENT );

    m_passes[FXAA_PASS] = std::make_shared<Render::PostProcessPass<1, 1>>(
    ...
    m_passes[TONEMAPPIN_PASS] =
        std::make_shared<Render::TonemappingPass>( m_device, m_shared, toneMappingPassConfig, DISPLAY_EXTENT, m_settings.displayColorFormat, !m_headless );
