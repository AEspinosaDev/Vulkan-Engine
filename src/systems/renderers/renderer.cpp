...
void BaseRenderer::on_before_render( Core::Scene* const scene ) {
    ...
    m_gpuScene.build( m_device, &m_frames[m_currentFrame], scene, DISPLAY_EXTENT, m_settings.enableRaytracing, m_settings.softwareAA == SoftwareAA::TAA );
...
void BaseRenderer::on_after_render( RenderResult& renderResult, Core::Scene* const scene ) {
    ...
    // Core::GPUSceneBuilder::prevViewProj = camera->get_projection() * camera->get_view();
...
void BaseRenderer::init_resources() {
    ...
    for ( size_t i = 0; i < m_frames.size(); i++ )
    {
        ...
        const size_t globalStrideSize =
            ( m_device->pad_uniform_buffer_size( sizeof( Core::Camera::GPUPayload ) ) + m_device->pad_uniform_buffer_size( sizeof( Core::Scene::GPUPayload ) ) );
        ...
        const size_t objectStrideSize =
            ( m_device->pad_uniform_buffer_size( sizeof( Core::Object3D::GPUPayload ) ) + m_device->pad_uniform_buffer_size( sizeof( Core::IMaterial::GPUPayload ) ) );
    ...
    m_shared = std::make_shared<Render::GPUResourcePool>();
    m_shared->init( m_device );
    ...
    m_shared->register_image( "BlueNoise", samplerText );
}
