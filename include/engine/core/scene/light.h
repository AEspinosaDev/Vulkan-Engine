...
class Light : public Object3D
{
  ...
  public:
    Light(std::string name, LightType type, Vec3 color = Vec3(1.0f, 1.0f, 1.0f), float intensity = 1.0f)
        : Object3D(name, LIGHT), m_color(color), m_intensity(intensity), m_lighType(type)
    {
    }
    ...
    virtual inline int get_shadow_pcf_kernel() const
    {
        return m_shadow.pcfKernel;
    }
    virtual inline void set_shadow_pcf_kernel(int k)
    {
        m_shadow.pcfKernel = k;
    }
...
class DirectionalLight : public Light
{
  ...
  public:
    DirectionalLight(Vec3 direction, Vec3 color = Vec3(1.0f, 1.0f, 1.0f), float intensity = 1.0f)
        : Light("Directional Light#" + std::to_string(DirectionalLight::m_instanceCount), LightType::DIRECTIONAL, color,
                intensity),
