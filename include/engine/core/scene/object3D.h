...
struct Transform
{
  ...
  public:
    Transform(Mat4 worldMatrix = Mat4(1.0f), Vec3 rotation = Vec3(0.0f), Vec3 scale = Vec3(1.0f),
              Vec3 position = Vec3(0.0f), Vec3 up = Vec3(0.0f, 1.0f, 0.0f), Vec3 forward = Vec3(0.0f, 0.0f, 1.0f),
              Vec3 right = Vec3(1.0f, 0.0f, 0.0f)

                  )
        : position(position), scale(scale), rotation(rotation), up(up), forward(forward), right(right),
...
class Object3D
{
  ...
  public:
    Object3D(const std::string na, ObjectType t) : TYPE(t), m_name(na), enabled(true), m_parent(nullptr)
    {
        m_transform.position = Vec3(0.0f);
    }

    Object3D(const std::string na, Vec3 p, ObjectType t) : TYPE(t), m_name(na), enabled(true), m_parent(nullptr)
    {
        m_transform.position = p;
    }

    Object3D(Vec3 p, ObjectType t) : TYPE(t), m_name(""), enabled(true), m_parent(nullptr)
    {
        m_transform.position = p;
    }
    Object3D(ObjectType t) : TYPE(t), m_name(""), enabled(true), m_parent(nullptr)
    {
        m_transform.position = Vec3(0.0f);
    }
