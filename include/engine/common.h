...
{
  public:
    template <typename... Args> VKException(const char *fmt, const Args &...args) : std::runtime_error(fmt)
    {
    }
};
...
typedef enum SyncType
{
    ...
    RELAXED_V_SYNC =
        VK_PRESENT_MODE_FIFO_RELAXED_KHR, // V-Sync with a wait time. If wait time is not enough potential tearing
} SyncType;
...
struct Extension
{
    ...
    Extension(const char *name, bool isOptional = false, void *pointerFeatureStruct = nullptr,
